...
size $O(\max(N_s, m))$. 
\UPDATED{Towards improving the efficiency in the distributed setting, usage of secure multiplication can be reduced. Suppose for a gate $\mathsf{g}$ among all the provers, $\prover_1, \ldots, \prover_{\Num}$, only $P_1$ and $P_2$ has input to $\mathsf{g}$. Thus the shares of the remaining provers corresponding to the gate $\mathsf{g}$ is $0$. Hence running an MPC between $P_1$ and $P_2$ maybe sufficient. Then resharing the outcome among all the provers is required. However, this approach requires end-to-end formal analysis. Moreover, this is circuit specific and maybe hard to implement.}