...
\begin{itemize}
	...
	%-------
	\item Among all the provers, $t$ are corrupt and try to learn about the honest provers' secrets. A $\DPZK$ protocol is said to have \textit{Witness Hiding (WH)} property if it is secure against this corruption scenario.
	%-------
	\item A corrupt verifier colludes with $t$ corrupt provers and tries to learn honest provers' secrets. A $\DPZK$ protocol is said to have \textit{Witness Hiding with Collusion (WHwC)} property if it is secure against this corruption scenario.
	%-------
\end{itemize}
%-------
...
%	%-------
%	
%	\CHANGED{The converse part follows directly from the proof of Soundness with Witness Extraction of Lemma~\ref{lemma:generic_dpzk}.}
%%----
...
%	%-------
%	
%	\CHANGED{The argument for the converse part is the same as the proof of zero-knowledge of Lemma~\ref{lemma:generic_dpzk}.}
...
\CHANGED{\begin{lemma}\label{lem:simdpgraphene}
...
%-------
\begin{proof}
\CHANGED{From the Lemma~\ref{lem:simgraphene}, we know that $\name$ has the zero-knowledge property. Coupled with Lemma~\ref{lemma:generic_dpzk}, this ensures the zero-knowledge property of $\dpname$. Thus there exists a polynomial-time simulator that generates a transcript which is indistinguishable from the real transcript.}
\end{proof}}
%-------
%-------------------------

\noindent\CHANGED{\textit{\textbf{Witness Hiding and Witness Hiding with Collusion:}} Our work is mainly concerned with building a public coin proof system, specifically NIZK so that anyone can verify the proof, and any prover can play a verifier's role yet learn nothing. Therefore witness hiding and witness hiding with collusion are the same in this setting.} 
%-------

\CHANGED{Though the proof of the witness Hiding property is similar to the witness Hiding proof of Lemma~\ref{lemma:generic_dpzk}, we provide detailed proof for our protocol here.}

\CHANGED{We will separate the provers into two classes: honest($H$) and corrupt($C$). Note that $|H|+|C|=\Num$ and $|C|<t$, for some $t<\Num$. In linear check, since there is no interaction among the provers other than the aggregator, it suffices to generate the view of the aggregator. For the quadratic check, we need that the multiplications among the provers are executed securely.} 
%-------
\CHANGED{\begin{lemma}\label{lem:WHlin}
...
%-------
\begin{proof}
	\CHANGED{Let $C$ be the set of all corrupt provers and $H$ be the set of all honest provers.}
	%-------
	
	\CHANGED{Case 1: Let the output of the protocol be ``reject''.$\Sim$ picks all the components of the view of corrupt parties uniformly at random from the appropriate domains such that it is indistinguishable from an honest execution.}
	%-------
	
	\CHANGED{Case 2: Let the output of the protocol be ``accept''. Then input for $\Sim$ consists of $\shr{\ewit} \; \forall \xi\in C$ and shares of $0^{2m-1}$. Without loss of generality, we can assume that there are 2 parties, $C$ and $H$. Also, assume that the adversary controls the aggregator. The view of $\Adv$ consists of:}
	%-------
	...
	\CHANGED{$\Sim$ does the following: 
	%-------
	\begin{itemize}
		%-------
...
\CHANGED{The distributed quadratic check protocol described in Figure~\ref{fig:distquadcheck} has witness hiding property if the multiplication of the secrets held by the provers is performed securely. Suppose $\Pi_{\sf Mult}$ be a protocol that securely realizes $\FMult$ functionality, which can withstand $t$ corrupt parties. Distributed quadratic check has witness hiding property against $t$ corrupt provers. The following lemma ensures the claim. 
%-------
\begin{lemma}\label{lem:WHquad}
...
\begin{proof}
	...
	\begin{itemize}
		%-------
		\item[--] \CHANGED{$\Sim$ picks $\rho, r, Q, \tau, \gamma, \beta, \delta, \beta_x, \beta_y, \beta_z$ uniformly at random from their respective domains.}
		%-------
		\item[--] \CHANGED{$\Sim$ picks $\ewit_a[\cdot,\cdot, k_u]\in \rsc{\alpha}{h,2m-1}$ for all $a\in\{x,y,z\}$  and computes $\honshr{\ewit_a}[\cdot,\cdot,k_u]= \ewit_a[\cdot, \cdot, k_u] - \corshr{\ewit_a}[\cdot, \cdot, k_u]$ $\forall a\in\{x,y,z\},\; u\in[t]$.}
		%-------
		\item[--] \CHANGED{$\Sim$ computes $\honshr{\comoracle_a}[\cdot,k_u]$ which is commitment of $\honshr{\ewit_a}[\cdot, \cdot,k_u]$ $\forall u\in[t], a\in\{x,y,z\}$.}
		%-------
		\item[--] \CHANGED{$\Sim$ computes $\honshr{\tilde{\ewit}}[\cdot,k_u]=\sum_{i\in[p]} \rho_i \honshr{\ewit_x}[i,\cdot,k_u]+ \rho_{p+i} \honshr{\ewit_y}[i,\cdot,k_u]+ \rho_{2p+i} \honshr{\ewit_z}[i,\cdot,k_u]$ and $\honshr{\tilde{c_{k_u}}}$, which is commitment of $\honshr{\tilde{\ewit}}[\cdot,k_u]$, for all $u\in [t]$.}
		%-------
		\item[--] \CHANGED{$\Sim$ picks $\honshr{\tilde{c_1}}, \ldots, \honshr{\tilde{c}_{\ell}}$ such that 
		%-------
		$\honshr{\tilde{c}_{k_u}} = \prod_{a\in[\ell]} (\honshr{\tilde{c}_{a}})^{\Lambda_{n,\ell}^T[a,k_u]} \; \forall u\in[t]$.
		$\Sim$ can pick such $\tilde{c}$ efficiently due to the fact that $\Lambda_{n,\ell}^T$ is a full rank matrix.}
		%-------
		\item[--] \CHANGED{$\Sim$ picks $\honshr{\comoracle_a}[\cdot, k]$ such that}
		
		\CHANGED{$\honshr{\tilde{c_k}} = \prod_{i\in[p]} (\honshr{\comoracle_x}[i,k])^{\rho_i}\cdot(\honshr{\comoracle_y}[i,k])^{\rho_{p+i}}\cdot(\honshr{\comoracle_z}[i,k])^{\rho_{2p+i}}$ $\forall k\notin\{k_u:u\in[t]\}$.}
		%-------
		\item[--] \CHANGED{$\Sim$ sends $\honshr{\comoracle_a}$ for all $a\in\{x,y,z\}$ and $\honshr{\tilde{c_1}}, \ldots, \honshr{\tilde{c}_{\ell}}$ to the aggregator.}
		%-------
		\item[--] \CHANGED{$\Sim$ picks $z$ from $\FF^{2m-1}$ such that $z[j]=0\; \forall j\in[m]$. Then $\Sim$ splits $z$ into $\honshr{z}$ and $\corshr{z}$ such that $\honshr{z}+\corshr{z} = z$.}
		%-------
		\item[--] \CHANGED{$\Sim$ computes $\corshr{\ewit_x\cdot\ewit_y}[\cdot,\cdot,k_u]$ from $\corshr{\ewit_x}, \corshr{\ewit_y}$ and $\honshr{\ewit_x}[\cdot,\cdot,k_u], \honshr{\ewit_y}[\cdot,\cdot,k_u]$ and picks $\corshr{\ewit_x\cdot\ewit_y}[i,\cdot,k]$ $\in \rsc{\alpha}{h,2m-1}$ uniformly for $k\notin \{k_u:u\in[t]\}$ and compute such that}
		
		\CHANGED{$\{(\sum_{i\in[p]}r_i\corshr{\ewit_x\cdot\ewit_y}[i,\cdot,\cdot])\varphi\}[j]=z[j] -\honshr{z}[j]+\{(\sum_{i\in[p]}r_i\corshr{\ewit_z}[i,\cdot,\cdot])\varphi\}[j]$ $\forall j\in[m]$.
		It is easy to see that picking such $\corshr{\ewit_x\cdot\ewit_y}[\cdot,\cdot,k]$ can be done efficiently.}
		%-------
		\item[--] \CHANGED{$\Sim$ calls $\Sim_{M}$ on input $\corshr{\ewit_x}, \corshr{\ewit_y}$ and $\corshr{\ewit_x\cdot\ewit_y}$ and gets a valid transcript of the interaction among the provers.}
		%-------
		\item[--] \CHANGED{$\Sim$ computes $\honshr{P}[\cdot,k_u]$ from $\corshr{\ewit_x}$, $\corshr{\ewit_y}$ and }
		
		\noindent\CHANGED{$\honshr{\ewit_x}[\cdot,\cdot,k_u]$, $\honshr{\ewit_y}[\cdot,\cdot,k_u]$ for all $u\in[t]$ and $\honshr{c_{k_u}}$, commitments of $\honshr{P}[\cdot,k_u]$.}
		
		\item[--] \CHANGED{$\Sim$ picks $\honshr{c_0}$, $\honshr{d_0}$ uniformly and picks
		$\honshr{c_1}, \ldots, \honshr{c_{2\ell-1}}$ such that:\\
		$\honshr{c_{k_u}} = \prod_{a\in[2\ell-1]}(\honshr{c_a})^{\Lambda_{n,2\ell-1}^T[a,k_u]}$ $\forall u\in[t]$\\
		$\cm = (\honshr{c_0})^{\beta}\prod_{a\in[2\ell-1]}(\honshr{c_a})^{\varphi_a}$.}
		%-------
		\item[--] \CHANGED{$\Sim$ sends $\honshr{c_0}, \honshr{c_1}, \ldots, \honshr{c_{2\ell-1}}, \honshr{d_0}$ to the aggregator.}
		%-------
		\item[--] \CHANGED{$\Sim$ sends $\honshr{\ewit_a}[\cdot,j_u,k_u]$ for all $a\in\{x,y,z\}$ and $\honshr{P}[\cdot,k_u]$, as responses.}
		%-------
		\item[--] \CHANGED{$\Sim$ finally sends $\honshr{z}$ and $\honshr{V_u}$ corresponding to the challenges $\delta$ and $\beta_x, \beta_y, \beta_z$.}
		%-------
	\end{itemize}
	\CHANGED{The view generated by $\Sim$ is indistinguishable from a transcript of an honest execution.}
\end{proof}
%-------
\CHANGED{Since $\dpname$ is the combination of linear check and quadratic check, simulator for the complete protocol follows the same strategies as the simulators described in Lemma~\ref{lem:WHlin} and \ref{lem:WHquad}. Hence, the $\dpname$ (Figure~\ref{fig:dpgraphene}) has the witness hiding property.}
