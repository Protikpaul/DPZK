...
\UPDATED{ \dpname achieves the following efficiency, where the notations are the same as above and $\mathsf{MPC}(x,y,z,w)$ denotes the cost of running a secure evaluation among $z$ parties with $w$ corruption for $x$ many multiplication gates with depth $y$.
\begin{itemize}
	\item {\em Number of rounds}: $\zkrounds=O(\log{N})$.
	\item {\em Argument size}: $\zkcomm= 4pt\bitsF$
	$+(4pt+8t\log{m}+4\ell+s+8t+4)\bitsG$.
