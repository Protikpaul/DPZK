We now describe the distributed protocol to produce a $\name$ proof for a
...
%--------------------------------------------------------
%We show that the messages received by the aggregator in both the distributed protocols can be efficiently simulated, independent of their views in the preceding MPC protocols, provided the parties follow the protocol and jointly possess a valid witness. We present the proofs of (i) Soundness with Witness Extraction (SoWE), (ii) Zero-Knowledge (Zk), and (iii) Witness-Hiding with Collusion (WHwC), which is the same as the plain Witness Hiding in our setting, in Appendix~\ref{app:dp_grapehene_securityproofs}. 
%---------------------------------------------------------------