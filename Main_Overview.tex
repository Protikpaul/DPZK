...
This indicates the number of MPC executions between the provers during the distributed proof generation. \commentA{the prior and the next sentences are same. please combine. it is not clear why the rounds inside MPC is not counted. or that what you mean? these descriptions are very confusing } %Informally, it denotes the number of MPC protocols the provers need to invoke. %separate MPCs that the provers have to run throughout the proof generation process, times the number of interactions needed inside each 
...
protocols. 
\UPDATED{Bulletproofs' \commentA{not sure if the "`" is used correctly. check}~\cite{bulletproofs} rangeproof provides a property that allows multiple proofs to be aggregated even if the provers are different.  \commentA{something wrong with this sentence. please correct}However, it does not work in our setting, that is, if the witness is held by multiple provers and jointly they generate the proof, whereas in bulletproofs, each provers must know the witness corresponding to its statement.}
...
Pinocchio  \cite{pinnochio_PHGR} but only for honest majority and semi-honest setting. Moreover, it needs the trusted setup
model. 
