...
This indicates the number of MPC executions between the provers during the distributed proof generation. Informally, it denotes the number of MPC protocols the provers need to invoke. %separate MPCs that the provers have to run throughout the proof generation process, times the number of interactions needed inside each 
...
protocols. 
\UPDATED{Bulletproofs'~\cite{bulletproofs} rangeproof provides a property that allows multiple proofs to be aggregated even if the provers are different. However, it does not work in our setting, that is, if the witness is held by multiple provers and jointly they generate the proof, whereas in bulletproofs, each provers must know the witness corresponding to its statement.}
